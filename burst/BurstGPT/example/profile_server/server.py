...
from profile_server.kvcache_wild import Query_wild
import os
import sys
import time
import json
...
class ServerBase(object):
    def __init__(self, model_path, data_path, backend,
        ...
        if config.prompt_config.get('workload') == 'burst':
...
class ServerOnline(ServerBase):
    ...
    def __init__(self, model_path, data_path, backend="vllm",
        ...
        self.workload = config.prompt_config.get('workload')
        ...
        elif self.workload == "wild":
            self.wild_queries = Query_wild(
                qps=self.qps, data_path=self.data_path, conv_or_api=self.conv_or_api, scale=self.scale)
        else:
            assert False
    ...
    async def issue_queries(self):
        ...
        for _ in range(surplus_prompts_num):
            ...
            if self.workload == "burst":
                ...
                _prompt, in_num, out_num, sampled_in_num, sampled_out_num, _, _sleep_time = self.wild_queries.get_query()
            else:
                assert False
